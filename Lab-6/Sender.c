...
int main()
{
    ...
    char *str =(char*) shmat(shmid,(void*)0,0);  //shmat attaching to shared memory
    ...
    //Receiving the reply msg from receiver  
    int n;
    printf("press 1 to continue: ");
    scanf("%d",&n);
    if(n==1){
   
    key_t key2=ftok("shmfile",-696);  //creating key to get reply from receiver
 
    char *str2=(char*) shmat(shmid,(void*)0,0);      
    printf("Data read: %s\n",str2);
   
    shmdt(str2);                                    //clearing shared memory
          shmctl(shmid,IPC_RMID,NULL);
    }
return 0;
}